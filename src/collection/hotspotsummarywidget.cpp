...
void HotspotSummaryWidget::setFile(const std::string& filePath, const std::string& displayName)
{
    ...
    for (const auto& [key, linePtr] : SourceLine::all_lines) {
        ...
        if (colonPos != std::string::npos) {
            keyPath = keyPath.substr(0, colonPos);
        }
    ...
    if (lineLatencies.empty()) {
        qDebug() << "ERROR: No hotspots found for path:" << QString::fromStdString(filePath);
    }

    // Sort and take top hotspots
    std::sort(lineLatencies.begin(), lineLatencies.end(),
    ...
    for (size_t i = 0; i < numLines; i++) {
        ...
        if (lineIt != foundLines.end() && lineIt->second) {
            ...
            for (int typeIdx = 0; typeIdx < linePtr->hotspot.latency.size() &&
                                  typeIdx < hotspotLine.latencyByType.size(); typeIdx++) {
...
void HotspotSummaryWidget::paintEvent(QPaintEvent* event)
{
    ...
    if (cleanFileName.startsWith("source_")) {
        ...
        if (firstUnderscorePos != -1) {
            ...
            if (secondUnderscorePos != -1) {
                cleanFileName = cleanFileName.mid(secondUnderscorePos + 1);
            }
        }
    }
    ...
    int lineTextWidth = std::max(contentFm.horizontalAdvance("Line 9999: "), // Handle large line numbers
                                 contentFm.horizontalAdvance("Line")) + padding;
    ...
    for (const auto& line : hotspotLines) {
        ...
        // Draw cycles count
        QString latencyText;
        if (line.latency > 1000000) {
            latencyText = QString("%1M").arg(line.latency / 1000000.0, 0, 'f', 1);
        } else if (line.latency > 1000) {
            latencyText = QString("%1K").arg(line.latency / 1000.0, 0, 'f', 1);
        } else {
            latencyText = QString::number(line.latency);
        }
